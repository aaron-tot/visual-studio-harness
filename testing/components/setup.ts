...
export async function setupSession(
  ...
  // Reveal + pin sidebar open for the rest of the test
  await page.mouse.move(2, 200);
  await page.waitForTimeout(600);
  const sidebarPin = page.locator("[data-testid='sidebar-pin']");
  await sidebarPin.waitFor({ state: "visible", timeout: 5000 });
  const pinTitle = (await sidebarPin.getAttribute("title")) ?? "";
  if (pinTitle !== "Unpin sidebar") {
  ...
  // Fail immediately if any chat error banner appears under a message bubble
  result.loop.register("no_chat_error", async () => {
    const els = page.locator("[data-testid='chat-error']");
    const count = await els.count();
    if (count > 0) {
      const texts: string[] = [];
      for (let i = 0; i < count; i++) {
        const t = await els.nth(i).innerText().catch(() => "(unreadable)");
        texts.push(t.replace(/\s+/g, " ").trim());
      }
      throw new Error(
        `Chat error element(s) detected (${count}) [data-testid=chat-error]:\n${texts.join("\n---\n")}`
      );
    }
  }, true);

  // Start the loop (~50ms scan)
  result.loop.begin(50);
