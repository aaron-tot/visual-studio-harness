...
import { listAudits, listSessions, listWorkspaces } from "../../../lib/api";
import type { AuditGroup, PlanScope } from "../types";
...
export function useAudits({
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (scope === "project") {
        ...
        const loaded = await Promise.all(
          [...roots].map(async (root) => {
            const audits = await listAudits({ scope: "project", workspaceRoot: root });
            return {
              key: `project:${root}`,
        ...
        const others = loaded
          .filter((g) => !g.isCurrent && g.audits.length > 0)
          .sort((a, b) => a.label.localeCompare(b.label));

        const next: AuditGroup[] = [];
      ...
      const loaded = await Promise.all(
        [...ids].map(async (sid) => {
          const audits = await listAudits({ scope: "session", sessionId: sid });
