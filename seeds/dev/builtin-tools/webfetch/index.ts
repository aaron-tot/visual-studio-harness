 ...
 * timeoutDefaultSec), clamped with the same min/max defaults as the original
 * tool. Uses global `fetch` + `Buffer` and wires an AbortController to
 * `ctx.abortSignal`. Only `ctx` + globals are used — no harness internals.
 */
...
export async function execute(
  ...
  const timeoutSec = Math.max(
    min,
    Math.min(max, typeof args.timeout === "number" ? args.timeout : def)
  );
